...
void ls_l(){
  ...
  for (uint i = 0; i < virtual_disk_sos->super_block.number_of_files ; i++){
    ...
    printf("   -%3s ", uright);
    printf("%3s  ", oright);
    printf("%15s  ", search_id((int)virtual_disk_sos->inodes[i].uid));
    printf("%5d  ", virtual_disk_sos->inodes[i].size);
    printf("%4d  ", virtual_disk_sos->inodes[i].nblock);
    printf("%5d  ", virtual_disk_sos->inodes[i].first_byte);
